...
## reformat table and remove unknown assignments 
```{r}
...
after manually looking through the assignments and remove any marine mammal not in study area that will mess with the assignment level.  
not worring about non-marine mammals, other marine species here. 
```{r}
nf_keepers_filtered <- nf_keepers %>%
  filter(!species == "Pusa sibirica")

nf_singletons <- nf_keepers_filtered %>%
  select(qseqid) %>%
  tally() %>%
  filter(n == 1)
...
nf_single_genus <- nf_keepers_filtered %>%
...
nf_single_family <- nf_keepers_filtered %>%
...
## now look at the fish and figure out what fish taxa are in/out of our range using fishbase
```{r}
